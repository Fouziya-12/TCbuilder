...
mydb= mysql.connector.connect(user='root', password='Fouzi@25',host='localhost',database='tc')
Session(app)
# mysql=MySQL(app)
...
def adminpannel():
    ...
    new = cursor.fetchall()
    cursor.close()
    return render_template('adminpannel.html',new=new)
...
def userpanel():
    ...
    if user_id:
        ...
        cursor.close()
        if new:
            return render_template('userpanel.html', new=new)
        else:
        ...
        if result is not None:
            ...
            if approval_status == "Approved":
                if request.method == 'POST':
                    ...
                    email = request.form['email']
                    cursor = mydb.cursor(buffered=True)
                    cursor.execute('INSERT INTO student_info (name,dob, gender, father, mother,nationality, belong, admission, classes, froms, tos, studies, dues, gc, certificate, reason, remarks,email) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)',(name,dob,gender, father, mother,nationality, belong, admission, classes, froms, tos,studies, dues, gc, certificate, reason, remarks,email))
                    mydb.commit()
                    cursor.close()
